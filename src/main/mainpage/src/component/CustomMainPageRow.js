...
import chksrc0 from '../asset/img/check0.svg';
import chksrc1 from '../asset/img/check1.svg';
import delsrc1 from '../asset/img/delIcon.svg';


const CustomMainPageRow = ({title, value, $mode, onChange, $page, pattern, 
  $inputTitle, $isButtonVisible, status, toggleStatus, delRow, changeRow}) => {
  // title : 제목
  // value : 내용
  // $mode : 상태 전환
  // onChange : 이벤트
...
display: ${props=>(props.$isButtonVisible ? "flex" : "none")};
padding: 0 0.5rem;
visibility : hidden;
${OutDiv}:hover & {
  visibility: visible;
...
`;
  return (
    <OutDiv $page={$page}>
      { status ? <Img src={chksrc1} alt="Checked" $isButtonVisible={$isButtonVisible} onClick={toggleStatus}></Img>
       : <Img src={chksrc0} alt="Unchecked" $isButtonVisible={$isButtonVisible} onClick={toggleStatus}></Img> }
      ...
      <Button $isButtonVisible={$isButtonVisible} onClick={changeRow}>수정</Button>
      <DelImg src={delsrc1} $isButtonVisible={$isButtonVisible} onClick={delRow}></DelImg>
    </OutDiv>
  );
};
